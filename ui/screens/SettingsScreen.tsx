...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans animate-fade-in ${isDarkTheme ? 'bg-black text-white' : 'bg-gray-100 text-black'}`}>
      
      {/* --- HEADER --- */}
      <header className={`px-4 py-4 flex items-center sticky top-0 z-20 border-b ${isDarkTheme ? 'bg-black border-zinc-900' : 'bg-white border-gray-200'}`}>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                          ...
                          <span className="text-lg">Smart recognition</span>
                      </div>
                      <Toggle switchOn={isSmartRecognitionOn} onToggle={toggleSmartRecognition} accentColor={accentColor} />
                  </div>

                  {/* Pop up view */}
                  <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg>
                          <span className="text-lg">Pop up view</span>
                      </div>
                      <Toggle switchOn={isPopupViewOn} onToggle={togglePopupView} accentColor={accentColor} />
                  </div>

                  {/* AI Support */}
                  <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                          <span className="text-lg">Ai support</span>
                      </div>
                      <Toggle switchOn={isAiSupportOn} onToggle={toggleAiSupport} accentColor={accentColor} />
                  </div>
          ...
          <section>
               <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-3">
                         ...
                         <div className="relative" ref={freqRef}>
                            ...
                            {showBackupFreq && (
                                <div className={`absolute top-full right-0 mt-1 z-10 w-40 shadow-xl border ${isDarkTheme ? 'bg-black border-zinc-700' : 'bg-white border-gray-300'}`}>
                                    {['Off', 'Every Day', 'Every 15 days', 'Every Month'].map(opt => (
                                        <button 
                                            key={opt}
                                            onClick={() => { setAutoBackupFrequency(opt); setShowBackupFreq(false); }}
                                            className={`w-full text-left px-3 py-2 text-sm hover:opacity-80 ${isDarkTheme ? 'text-white hover:bg-zinc-800' : 'text-black hover:bg-gray-100'} ${autoBackupFrequency === opt ? (isDarkTheme ? 'bg-zinc-800' : 'bg-gray-100') : ''}`}
                                        >
                                            {opt}
                                        </button>
                                    ))}
                                ...
                                <div className={`absolute top-full right-0 mt-1 z-10 w-40 shadow-xl border ${isDarkTheme ? 'bg-black border-zinc-700' : 'bg-white border-gray-300'}`}>
                                    {['Google', 'Local files'].map(opt => (
                                        <button 
                                            ...
                                            className={`w-full text-left px-3 py-2 text-sm hover:opacity-80 ${isDarkTheme ? 'text-white hover:bg-zinc-800' : 'text-black hover:bg-gray-100'} ${backupDestination === opt ? (isDarkTheme ? 'bg-zinc-800' : 'bg-gray-100') : ''}`}
                                        >
                                            {opt}
                                        </button>
                                    ))}
           ...
           <section className="space-y-6 pt-4 border-t border-gray-800">
               <div className="flex items-center space-x-3">
                   <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                      ...
                      {sortedLanguages.map((lang, index) => {
                          ...
                          return (
                              <React.Fragment key={lang}>
                                  ...
                                  {/* Separator after selected item if it's the first item */}
