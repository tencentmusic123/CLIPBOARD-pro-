...
const TrashScreen: React.FC<TrashScreenProps> = ({ onBack }) => {
  ...
  // Unique Tags from items for Filter UI
  ...
  return (
    <div className="min-h-screen bg-black text-white relative flex flex-col max-w-md mx-auto border-x border-zinc-900 shadow-2xl h-full animate-fade-in font-sans">
      ...
      <header className="px-4 py-4 flex items-center justify-between bg-black/95 backdrop-blur-md sticky top-0 z-20 border-b border-zinc-800 h-16">
        <div className="flex items-center">
            ...
            <div className="flex items-center space-x-2">
                ...
                <h2 className="text-gold text-2xl tracking-wider font-normal">
        ...
        {isSelectionMode ? (
             <div className="flex items-center space-x-3 text-sm font-medium">
                 <button onClick={handleSelectAll} className="text-white hover:text-gold whitespace-nowrap">
                     {selectedIds.size === filteredItems.length && filteredItems.length > 0 ? 'Deselect All' : 'Select All'}
                 </button>
                 <button onClick={handleDeleteSelected} className="text-white hover:text-red-500">Delete</button>
                 <button onClick={handleRestoreSelected} className="text-white hover:text-green-400">Restore</button>
             </div>
        ) : (
            <button 
      ...
      <main className="flex-1 px-4 py-6 overflow-y-auto no-scrollbar relative z-0">
          {loading ? (
             <div className="text-gold text-center mt-20 font-mono">Loading Trash...</div>
...
const FilterItem: React.FC<{ label: string; icon?: React.ReactNode; active: boolean; onClick: () => void; underline?: boolean }> = ({ label, icon, active, onClick, underline }) => (
    <button 
        onClick={onClick}
        className={`flex items-center justify-center space-x-1 py-1 transition-colors ${active ? 'text-white font-medium' : 'text-gray-400 hover:text-white'}`}
    >
        {icon}
        <span className="capitalize">{label}</span>
        {underline && active && <div className="absolute bottom-0 w-8 h-[2px] bg-gold translate-y-2"></div>}
        {underline && active && <div className="absolute h-0 w-0"></div>} {/* Dummy for positioning context if needed */}
    </button>
);
